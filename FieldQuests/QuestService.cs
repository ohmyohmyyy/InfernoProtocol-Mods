...
internal sealed class QuestService
{
    ...
    {
        _deaths.Remove(npc.GetInstanceID());
    }
    internal void Death(ANPC npc, DamageData damage)
    {
        ...
        if (player == null) player = ResolveSoleLocalHost();
    ...
    private static Player ResolveSoleLocalHost()
    {
        NetworkManager manager = NetworkManager.Singleton;
        if (manager == null || !manager.IsListening || !manager.IsServer || Player.localPlayer == null) return null;
        var clients = manager.ConnectedClientsList.Cast<Il2CppSystem.Collections.Generic.IReadOnlyCollection<NetworkClient>>();
        return clients.Count == 1 && Player.localPlayer.IsServer ? Player.localPlayer : null;
    }
