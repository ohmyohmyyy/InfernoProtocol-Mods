...
For manual installation, install BepInEx 6 IL2CPP and the [InfernoProtocolInteropFix](https://thunderstore.io/c/inferno-protocol/p/FrankMods/InfernoProtocolInteropFix/) compatibility patcher first. Close the game, extract the ContentPlus release ZIP into the game folder, and merge its BepInEx directory. Launch through the configured BepInEx loader and check for `ContentPlus 0.5.0 loaded` in BepInEx/LogOutput.log. The ZIP also contains Thunderstore metadata; it is harmless outside the plugins directory.
