...
- Branches use one MaskableGraphic mesh, retaining RectMask2D clipping. Its bounds expand with the genome so branches remain visible at the ends of a long tree.
- Static branches, nodes and animated highlights use separate child canvases. Only selected/hover-state transitions update node colors; only the selected highlight and two travelling points animate continuously.
- Closing hides the cached panel; reopening reuses its objects when data is unchanged. Native list/feature/localization notifications replace periodic hierarchy scans. Snapshot buffers are reused.
- Unload destroys the cached canvas and owned sprite textures. A lost scene canvas is rebuilt on the next open. Native panel/input restoration is unchanged.
- 4,380 assertions cover layout/navigation and segment width/orientation. Test masking while panning to both ends, reduced motion, repeated open/close, acquiring a gene while open, language changes and world reload. Confirm the log has no custom-graphic registration or rendering errors. No FPS gain has been measured yet.
...
The tree uses the remembered BetterUI color palette. Native fields are checked at open or following native update notifications; graphics rebuild only when the displayed data or palette changes. Geometry and cached circle textures are owned by the mod. No world searches or gene writes are performed. Closing restores the native panel's CanvasGroup and the input maps that were previously enabled; held navigation buttons must release before gameplay input returns.
