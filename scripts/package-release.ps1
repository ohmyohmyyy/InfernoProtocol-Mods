...
Compress-Archive -Path (Join-Path $betterUiStage '*') -DestinationPath (Join-Path $dist 'BetterUI-v0.9.2.zip')
Compress-Archive -Path (Join-Path $utilityWheelStage '*') -DestinationPath (Join-Path $dist 'UtilityWheel-v0.2.0.zip')
Copy-Item -LiteralPath $betterUiDll -Destination (Join-Path $dist 'InfernoProtocol.BetterUI-v0.9.2.dll')
